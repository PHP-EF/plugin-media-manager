...
trait SSO {
    ...
    private function initiateTautulliSSO($data) {
        ...
        if (isset($Results) && isset($Results['status']) && $Results['status'] == 'success') {
            if (isset($Results['token']) && isset($Results['uuid'])) {
                setcookie('tautulli_token_'.$Results['uuid'], $Results['token'], time() + (86400 * 30), ".dev.tmmn.local"); // 30 days
            }
            return $Results;
        } else {
            return false;
    ...
    private function initiateOverseerrSSO($data) {
        ...
        if (isset($Results->success)) {
            setcookie('connect.sid', $Results->cookies['connect.sid']->value, time() + (86400 * 30), ".dev.tmmn.local"); // 30 days
            return true;
        } else {
            return false;
        }
